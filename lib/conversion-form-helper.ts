import { volumeConversions, weightConversions, Unit, VolumeUnit, WeightUnit, TemperatureUnit } from './constants';

function checkOfType<T extends Unit>(units: ReadonlyArray<Unit>, enumObj: Record<string, T>): void {
  units.forEach((unit) => {
...
export function convertVolume(value: number, fromUnit: Unit, toUnit: Unit): number {
  ...
  const baseCups = value / volumeConversions[fromUnit as keyof typeof volumeConversions];
...
export function convertWeight(value: number, fromUnit: Unit, toUnit: Unit): number {
  ...
  const baseOunces = value / weightConversions[fromUnit as keyof typeof weightConversions];
  return baseOunces * weightConversions[toUnit as keyof typeof weightConversions];
}
